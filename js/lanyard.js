  ...
  function render(d) {
    ...
    if (clan && clan.tag) {
      ...
      if (clan.badge && clan.identity_guild_id) {
        clanBadgeEl.src =
          "https://cdn.discordapp.com/clan-badges/" +
          clan.identity_guild_id +
          "/" +
          clan.badge +
          ".png?size=16";
    ...
    if (d.listening_to_spotify && d.spotify) {
      ...
      var game = (d.activities || []).find(function (a) {
        return a.type === 0;
      });
      if (game) {
      ...
      if (msg.op === 1) {
        ...
        fetch("https://dcdn.dstn.to/profile/" + UID)
          ...
          .then(function (data) {
            ...
            namelineEl.querySelectorAll(".dc-badge").forEach(function (b) {
              b.remove();
            });

            badges.forEach(function (badge) {
              if ((CONFIG.hideBadges || []).indexOf(badge.id) !== -1) return; // ← überspringen
